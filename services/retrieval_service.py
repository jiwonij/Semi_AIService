...
class RetrievalService:
    ...
    _embeddings_cache: HuggingFaceEmbeddings | None = None  # ✅ 클래스 레벨 캐시 (프로세스 내 1회만 로드)
    ...
    def _get_embeddings(self) -> HuggingFaceEmbeddings:
        # ✅ 클래스 레벨 캐시 — 인스턴스가 새로 생성되어도 프로세스 내 1회만 로드
        if RetrievalService._embeddings_cache is None:
            RetrievalService._embeddings_cache = HuggingFaceEmbeddings(
                model_name=self.embedding_model_name,
                model_kwargs={
                    "device": "cpu",
                    "trust_remote_code": True,  # ✅ bge-m3 호환 필수
                },
                encode_kwargs={"normalize_embeddings": True},
            )
        return RetrievalService._embeddings_cache
